...
Fabricator(:legacy_contract, from: Pacto::Contract) do
...
Fabricator(:legacy_partial_contract, from: Pacto::Contract) do
  initialize_with { @_klass.new to_hash } # Hash based initialization
  name { 'Dummy Contract' }
  file { 'file:///does/not/exist/dummy_contract.json' }
  request { Fabricate(:request_clause).to_hash }
...
Fabricator(:legacy_request_clause, from: Pacto::RequestClause) do
  initialize_with { @_klass.new to_hash } # Hash based initialization
  host { 'example.com' }
  http_method { 'GET' }
  path { '/abcd' }
...
Fabricator(:legacy_response_clause, from: Pacto::ResponseClause) do
  initialize_with { @_klass.new to_hash } # Hash based initialization
  status { 200 }
  headers do
    {
...
Fabricator(:legacy_schema, from: Hashie::Mash) do
  transient :version
  initialize_with { @_klass.new to_hash } # Hash based initialization
  type { 'object' }
  required do |attrs|
...
Fabricator(:legacy_an_example, from: Hashie::Mash) do
  initialize_with { @_klass.new to_hash } # Hash based initialization
