    ...
    describe '.validate' do
      ...
      context 'when headers and body match and the ResponseStatusValidator reports no errors' do
        it 'does not return any errors' do
          ...
          #   and_return([])
          expect(Pacto::Validators::ResponseStatusValidator).to receive(:validate).with(expected_response.status, actual_response.status).and_return(nil)
          expect(Pacto::Validators::ResponseHeaderValidator).to receive(:validate).with(expected_response.headers, actual_response.headers).and_return(nil)
