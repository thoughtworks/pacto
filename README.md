...
```
"Upload Album Art": PUT /store/album/{album_id}/cover_art
"Download Album Art": GET /store/album/{album_id}/cover_art{?size}
```
...
Since Pacto has added a layer of abstraction you can use the log to generate semantically equivalent requests even as you're expirementing with changes. For example, if you change the routing for the service from:
```
"Upload Album Art": PUT /store/album/{album_id}/cover_art
```
to
```
"Upload Album Art": PUT /cover_art{?album_id}
```
then Pacto will automatically adjust the URLs.
